...
process(Window, ServerPid, Transaction) ->
    ...
    receive
	{proceed, TS, ServerPid} -> send(Window, ServerPid, Transaction, TS, 0),io:format("proceed~n"); %% received green light send the transaction.
	{close, ServerPid} -> exit(serverDied);
	Other ->
	    io:format("client active unexpected: ~p~n",[Other])
    end.
...
send(Window, ServerPid, [], TS, ID) ->
    ServerPid ! {confirm, self(), TS, ID}, %% Once all the list (transaction) items sent, send confirmation
    receive
...
send(Window, ServerPid, [H|T], TS, ID) ->
    sleep(3),
    case loose(0) of
	%% In order to handle losses, think about adding an extra field to the message sent
	false -> ServerPid ! {action, self(), H, TS, ID};
        true -> ok
    end,
    NextID = ID + 1,
    send(Window, ServerPid, T, TS, NextID).
%%%%%%%%%%%%%%%%%%%%%%% Active Window %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
